...
def build_parser():
    ...
    p.add_argument(
        ...
        required=True,
        type=str,
        help="Schema in Json string format to send to controller VM",
    )
    p.add_argument(
        "--cid", type=int, default=5, help="controller VM guest CID (default: 5)"
    )
    p.add_argument(
        "--port",
        type=int,
        default=7000,
        help="controller VM vsock listen port. (default: 7000)",
    )
