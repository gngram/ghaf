...
async def device_event_with_user_notification(context, config, device, upm_host):
    global userDevices
    if device.action == 'add':
        logger.debug("Device plugged: %s", device.sys_name)
        logger.debug("Subsystem: %s, path: %s", device.subsystem, device.device_path)
        ...
        if is_usb_device(device):
            ...
            try:
                ...
                permitted = res[1]
                if len(permitted) == 0:
                    vm = await vm_for_usb_device(context, config, None, usb_info, None, True)
...
async def monitor_loop(monitor, context, config, user_interface, watcher, attach_connected):
    notifyUser = isinstance(user_interface, HostService)
    while True:
        device = await asyncio.to_thread(monitor.poll, 1)
        if device:
            if notifyUser:
                await device_event_with_user_notification(context, config, device, user_interface)
            else:
...
async def async_main():
    ...
    await monitor_loop(monitor, context, config, svc, watcher, args.attach_connected)
